...
for SRR_dir in "$path"/SRR*/; do # "/" final para iterar solo en directorios
       #iteramos sobre cada archivo fastq 
       echo "Limpiando las muestras del SRR ${SRR_dir##/*}"
       shopt -s nullglob
       for SRR in "$SRR_dir"/*.fastq.gz; do 
              ...
              if [[ "$SRR" != *_2.fastq.gz ]];then
                     ...
                     if [[ "$SRR" == *_1.fastq.gz ]];then
                            ...
                            cutadapt -q 20 -m 25 -o "$output_clean"/"$output_1" -p "$output_clean"/"$output_2" $archivo_1 $archivo_2 #explorar la idea de usar qsub con un script externo como vero 
                     else 
                            echo "Limpiando archivo no pareado de ${SRR##/*} salida $output_1"
                            #se corre el cut adapt con un solo archivo (single end) 
                            cutadapt -q 20 -m 25 -o "$output_clean"/"$output_1" $archivo_1
                     fi
              fi
       done
done
